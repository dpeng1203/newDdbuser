...
export default {
    ...
    created() {
        if(localStorage.memId) {
            this.qrText = 'http://web2.dodohz.com/ddbxryuser/index.html#/signqr?memCode=' + localStorage.memId
            this.wxRegister() 
        }else{
            Toast('推荐人信息不存在，请重新登陆！')
            localStorage.clear()
            // this.$router.push('/')
            window.location.href = 'http://web2.dodohz.com/ddbxryuser/index.html#/'
        }
